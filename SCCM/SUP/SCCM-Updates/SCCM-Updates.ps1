<# 
.SYNOPSIS
Script para verificação a instalação remota de patches Microsoft através da interface WMI do agente do SCCM.
Desenvolvido por Vincius Haddad (J1KO)
...
param(
    ...
    [parameter(Mandatory=$false,ValueFromPipeline=$false,ParameterSetName = "Actions")][ValidateNotNullOrEmpty()]
...
process{
    ...
    Function StartUpdatesActions($ComputerName){
        try{
            ...
            StartAction $ComputerName $Actions.'Software Update Sotre'
    ...
    Function VerifyConnection($ComputerName){
        ...
        try{
            ...
            if(Test-Connection -computername $ComputerName -count 1 -quiet){
                ...
                elseif((Get-WmiObject win32_service -ComputerName $ComputerName -ErrorAction SilentlyContinue | Where-Object {$_.Name -eq "CcmExec"}).State -eq "Running"){
                    return $true
                }
                else{
                    return $false
    ...
    Write-host "SCCM Update Tool v$version"
        ...
        If (VerifyConnection $computer){
            ...
            elseif($PSCmdlet.ParameterSetName -ilike "Actions*"){
                ...
                if($ExecuteAllActions){ 
                    $status = StartAllActions $computer

                }
                else{
                    $status = StartUpdatesActions $computer
                }
