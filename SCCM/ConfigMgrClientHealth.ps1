...
Begin {
    ...
    function Write-Log {
        ...
        (
            ...
            [ValidateNotNullOrEmpty()]$Linha
        )
        $Arquivo = "\\S6006as1810\cliente$\ClientHealthLogs\$env:computername.log"
        If (Test-Path $Arquivo) { }else { $arquivo = New-Item -Path $Arquivo -ItemType "file" }
        $DataLog = (Get-Date).tostring("dd/MM/yyyy")
        $HoraLog = (Get-Date).tostring("HH:mm:ss")
        Add-Content -Path $Arquivo -Value "$DataLog $horalog : $Linha"  -Force
        Start-Sleep 0.6
    }
    ...
    If ((!$PSBoundParameters.ContainsKey('Config')) -and (!$PSBoundParameters.ContainsKey('Webservice'))) {
        ...
        Write-Log "No config provided, defaulting to $Config"
    }
    ...
    Function Test-XML {
        ...
        try {
            ...
            Write-Log "$xmlFilePath : $($_.toString())"
    ...
    Function Update-Webservice {
        ...
        try { Invoke-RestMethod -Method $Method -Uri $URI -Body $Obj -ContentType $ContentType | Out-Null }
            ...
            Write-Log "Error Invoking RestMethod $Method on URI $URI. Failed to update database using webservice. Exception: $ExceptionMessage"
    ...
    Function Get-ConfigFromWebservice {
        ...
        Write-Log "Retrieving configuration from webservice. URI: $URI"
        try {
            ...
            Write-Log "Error retrieving configuration from webservice $URI. Exception: $ExceptionMessage" -ForegroundColor Red
            Exit 1
        }
    ...
    Function Get-ConfigClientInstallPropertiesFromWebService {
        Param(
            ...
            Write-Log "Retrieving client install properties from webservice"
        try {
            ...
            Write-Log "Error retrieving client install properties from webservice $URI. Exception: $ExceptionMessage" -ForegroundColor Red
            Exit 1
        }
    ...
    Function Get-ServiceUpTime {
        ...
        Catch{
            ...
            Write-Log "The '$($Name)' service could not be found."
            Return
        }
        ...
        Catch {
            ...
            Write-Log "Could not get the uptime time for the '$($Name)' service from the event log.  Relying on the process instead."
        }
        ...
        Catch{
            ...
            Write-Log "Could not get the uptime time for the '$($Name)' service.  Returning max value."
            Return [int]::MaxValue
        }
    }
    ...
    Function Search-CMLogFile {
        ...
        :loop for ($i=($LogData.Count - 1);$i -ge 0; $i--) {
            ...
            try{
                ...
                Write-Log "Could not parse the line $($i) in '$($LogFile)': $($LogData[$i])"
				continue
            }
            ...
            If ($LogTime -lt $StartTime) {
                ...
                Write-Log "No log lines in $($LogFile) matched $($SearchStrings) before $($StartTime)."
                break loop
            }
    ...
    Function Test-CcmSQLCELog {
        ...
        if ( (Test-Path -Path $logFile) -and ($logLevel -ne 0) ) {
            ...
            if ( (($now - $FileDate).Days -lt 7) -and ((($now - $FileCreated).Days) -gt 7) ) {
                ...
                Write-Log $text -ForegroundColor Red
                # Delete *.SDF Files
                ...
                Write-Log "Waiting 10 seconds to allow file locking issues to clear up"
                ...
                try {
                    ...
                    Write-Log "Obviously that wasn't enough time"
    ...
    Function Test-InTaskSequence {
        ...
        if ($tsenv) {
            ...
            Write-Log "Configuration Manager Task Sequence detected on computer. Exiting script"
            Exit 2
        }
    }
    ...
    Function Get-OSDiskFreeSpace {
        ...
        Write-Log ([math]::Round($freeSpace,2))
    }
    ...
    Function Test-Update {
        ...
        Write-Log "Validating required updates is installed on the client. Required updates will be installed if missing on client."
        #$OS = Get-WmiObject -class Win32_OperatingSystem
        $OSName = Get-OperatingSystem
        ...
        Else {
            ...
            Write-Log "Updates Failed: Could not locate update folder '$($Updates)'."
        }
    }
    ...
    Function Test-ConfigMgrClient {
        ...
        if (Get-Service -Name ccmexec -ErrorAction SilentlyContinue) {
            ...
            if ($LocalDBFilesPresent -eq $False) {
                    ...
                    Write-Log "ConfigMgr Client database files missing. Reinstalling..."
            ...
            if ($testLocalDB -like "enable") {
                ...
                if ($LocalDB -eq $true) {
                    ...
                    Write-Log "ConfigMgr Client database corrupt. Reinstalling..."
                    $Reinstall = $true
                    $Uninstall = $true
                }
            }
            ...
            if (($CCMService.Status -eq "Stopped") -and ($LocalDB -eq $false)) {
                try {
                    ...
                    Write-Log "ConfigMgr Agent not running. Attempting to start it."
            ...
            Try {
                ...
                Write-Log 'Failed to connect to WMI namespace "root/ccm" class "SMS_Client". Clearing WMI and tagging client for reinstall to fix.'
    ...
    Function Test-UpdateStore {
        ...
        Write-Log "Check StateMessage.log if State Messages are successfully forwarded to Management Point"
        $logdir = Get-CCMLogDirectory
    ...
    Function Test-RegistryPol {
        ...
        Write-Log "Check WUAHandler.log for errors since $($StartTime)."
        $logdir = Get-CCMLogDirectory
        ...
        if ($Days) {
            ...
            Write-Log "Check machine registry file to see if it's older than $($Days) days."
            try {
        ...
        try {
            ...
            Write-Log "Checking the Group Policy event log for errors since $($StartTime)."
        ...
        if ($RepairReason -ne ""){
            ...
            Write-Log "GPO Cache: Broken ($RepairReason)"
            Write-Verbose 'Deleting registry.pol and running gpupdate...'
            Write-Log 'Deleting registry.pol and running gpupdate...'
            ...
            Write-Log 'Refreshing update policy'
            ...
            Write-Log "GPO Cache: $($log.WUAHandler)"
        }
        else {
            $log.WUAHandler = 'OK'
            Write-Output "GPO Cache: OK"
            Write-Log "GPO Cache: OK"
        }
    }
    ...
    Function Remove-CCMOrphanedCache {
        ...
        Write-Log "Clearing ConfigMgr orphaned Cache items."
        try {
            ...
            ForEach ($CachedFolder in $AllCachedFolders) {
                If ($ValidCachedFolders -notcontains $CachedFolder) {
                    ...
                    if ((Get-ItemProperty $CachedFolder).LastWriteTime -le (get-date).AddDays(-14)) {
                        ...
                        Write-Log "Removing orphaned folder: $CachedFolder - LastWriteTime: $((Get-ItemProperty $CachedFolder).LastWriteTime)"
    ...
    Function Resolve-Client {
        ...
        if ((Test-Path $ClientShare -ErrorAction SilentlyContinue) -eq $true) {
            ...
            Write-Log "Perform a test on a specific registry key required for ccmsetup to succeed."
            Test-CCMSetup1

            Write-Verbose "Enforce registration of common DLL files to make sure CCM Agent works."
            Write-Log "Enforce registration of common DLL files to make sure CCM Agent works."
            ...
            if ($Uninstall -eq $true) {
				...
				Write-Log "Trigger ConfigMgr Client uninstallation using Invoke-Expression."
				...
				do {
					...
					if (Get-Process "ccmsetup" -ErrorAction SilentlyContinue) {
						...
						Write-Log "ConfigMgr Client Uninstallation still running"
						$launched = $true
					}
					else { $launched = $false }
            ...
            Write-Log "Trigger ConfigMgr Client installation using Invoke-Expression."
            Write-Verbose "Client install string: $ClientShare\ccmsetup.exe $ClientInstallProperties"
            Write-Log "Client install string: $ClientShare\ccmsetup.exe $ClientInstallProperties"
			...
			do {
				...
				if (Get-Process "ccmsetup" -ErrorAction SilentlyContinue) {
					...
					Write-Log "ConfigMgr Client installation still running"
					$launched = $true
				}
				else { $launched = $false }
            } while ($launched -eq $true)
            ...
            if ($FirstInstall -eq $true) {
                ...
                Write-Log "ConfigMgr Client was installed for the first time. Waiting 6 minutes for client to syncronize policy before proceeding."
    ...
    Function Test-WMI {
        ...
        Try {
            ...
            Write-Log 'Failed to connect to WMI class "Win32_ComputerSystem". Voting for WMI fix...'
            $vote++
        } Finally {
            if ($vote -eq 0) {
                $text = 'WMI Check: OK'
                ...
                Write-Log "returning true to tag client for reinstall"
                $obj = $true
            }
            #Out-LogFile -Xml $xml -Text $text
            Write-Output $obj
    ...
    Function Test-RefreshComplianceState {
        ...
        Write-Log "The compliance states were last sent on $($LastSent)"
    ...
    Function Test-SMSTSMgr {
        ...
        if (($service.ServicesDependedOn).name -contains "ccmexec") {
            ...
            write-Log "SMSTSMgr: Removing dependency on CCMExec service."
        ...
        if (($service.ServicesDependedOn).name -notcontains "Winmgmt") {
            ...
            write-Log "SMSTSMgr: Adding dependency on Windows Management Instrumentaion service."
            start-process sc.exe -ArgumentList "config smstsmgr depend= winmgmt" -wait
        }
    ...
    Function Test-Service {
        ...
        if ($service.Status -eq "Running") {
            ...
            If ($Uptime){
                ...
                Write-Log "Verify the $($Name) service hasn't exceeded uptime of $($Uptime) days."
                ...
                if ($ServiceUptime -ge $Uptime) {
                    try {
                        ...
                        While ((Get-Process -Name WUSA,wuauclt,setup,TrustedInstaller,msiexec,TiWorker,ccmsetup -ErrorAction SilentlyContinue).Count -gt 0){
                            ...
                            If($MinutesLeft -le 0){
                                ...
                                Write-Log "Timed out waiting $($WaitMinutes) minutes for installation processes to complete.  Will not restart the $($Name) service."
                                $ProcessesStopped=$False
                                Break
                            }
                            Write-Warning "Waiting $($MinutesLeft) minutes for installation processes to complete."
                            Write-Log "Waiting $($MinutesLeft) minutes for installation processes to complete."
                            Start-Sleep -Seconds 30
                        }
                        $Timer.Stop()
                    ...
                    Write-Log "Service $($Name) uptime: OK"
                }
            }
        }
        else {
            if ($WMIService.Status -eq 'Degraded') {
                try {
                    ...
                    Write-Log "Identified $Name service in a 'Degraded' state. Will force $Name process to stop."
                    ...
                    Write-Log "Succesfully stopped the $Name service process which was in a degraded state."
                }
                Catch{
                    Write-Error "Failed to force $Name process to stop."
                    Write-Log "Failed to force $Name process to stop."
                }
            }
            try {
                $RetryService= $False
                ...
                If ($_.Exception.Hresult -eq '-2146233087'){
                    ...
                    Write-Log "Failed to start service $Name because it's sharing a thread with another process.  Changing to use its own thread."
    ...
    function Test-AdminShare {
        ...
        Write-Log "Test the ADMIN$ and C$"
    ...
    Function Start-Ccmeval {
        ...
        Write-Log "Starting Built-in Configuration Manager Client Health Evaluation"
    ...
    Function Update-SCCMLogFile {
        ...
        Write-Log "Start Update-SCCMLogFile"
        ...
        Write-Log "End Update-SCCMLogFile"
    }
    ...
    Function Test-SCCMHardwareInventoryScan {
        ...
        Write-Log "Start Test-SCCMHardwareInventoryScan"
        ...
        Write-Log "End Test-SCCMHardwareInventoryScan"
    }
    ...
    Function Test-PolicyPlatform {
        ...
        try {
            if (Get-WmiObject -Namespace 'root/Microsoft' -Class '__Namespace' -Filter 'Name = "PolicyPlatform"') { Write-Host "PolicyPlatform: OK" }
                ...
                Write-Log "PolicyPlatform: Not found, recompiling WMI 'Microsoft Policy Platform\ExtendedStatus.mof'"
    ...
    Function Test-SoftwareMeteringPrepDriver {
        ...
        Write-Log "Start Test-SoftwareMeteringPrepDriver"
        ...
        Write-Log "End Test-SoftwareMeteringPrepDriver"
    }
    ...
    function Test-SQLConnection {
        ...
        try
        {
            ...
            Write-Log "SQL connection test successfull"
        }
        catch {
            ...
            Write-Log "SQL connection test failed"
        }
        finally {Write-Output $obj }
    }
    ...
    Function CleanUp {
        ...
        if (((Test-Path "$clientpath\Temp" -ErrorAction SilentlyContinue) -eq $True) -and ($NoDelete -eq $false) ) {
            ...
            Write-Log "Cleaning up temporary files in $clientpath\ClientHealth"
        ...
        if (($LocalLogging -ne "true") -and ($NoDelete -eq $false)) {
            ...
            Write-Log "Local logging disabled. Removing $clientpath\ClientHealth"
    ...
    Function Test-ValuesBeforeLogUpdate {
        ...
        Write-Log "Start Test-ValuesBeforeLogUpdate"
        ...
        Write-Log "End Test-ValuesBeforeLogUpdate"
    }
    ...
    Function Update-LogFile {
        ...
        Write-Log "Start Update-LogFile"
...
Process {
    ...
    Write-Log "Starting precheck. Determing if script will run or not."
    ...
    If (-NOT ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator"))
    {
        ...
        Write-Log $StartupText1

        Write-Verbose "Determing if a task sequence is running."
        Write-Log "Determing if a task sequence is running."
        ...
        if ($tsenv -ne $null) {
            ...
            Write-Log "Task sequence $TSName is active executing on computer. ConfigMgr Client Health will not execute."
            Exit 1
         }
         else {
            $StartupText2 = "ConfigMgr Client Health " +$Version+ " starting."
            Write-Host $StartupText2
            Write-Log $StartupText2
         }
    }
    ...
    Write-Log "Script last ran: $($LastRun)"

    Write-Verbose "Testing if log files are bigger than max history for logfiles."
    Write-Log "Testing if log files are bigger than max history for logfiles."
    ...
    Write-Log 'Validating WMI is not corrupt...'
    $WMI = Get-XMLConfigWMI
    if ($WMI -like 'True') {
        Write-Verbose 'Checking if WMI is corrupt. Will reinstall configmgr client if WMI is rebuilt.'
        Write-Log 'Checking if WMI is corrupt. Will reinstall configmgr client if WMI is rebuilt.'
    ...
    Write-Log 'Determining if compliance state should be resent...'
    ...
    if ( ($RefreshComplianceState -like 'True') -or ($RefreshComplianceState -ge 1)) {
        ...
        Write-Log "Checking if compliance state should be resent after $($RefreshComplianceStateDays) days."
    ...
    Write-Log 'Testing if ConfigMgr client is installed. Installing if not.'
    Test-ConfigMgrClient -Log $Log

    Write-Verbose 'Validating if ConfigMgr client is running the minimum version...'
    Write-Log 'Validating if ConfigMgr client is running the minimum version...'
    ...
    Write-Log 'Validate that ConfigMgr client do not have CcmSQLCE.log and are not in debug mode'
    ...
    Write-Log 'Validating client cache size. Will restart configmgr client if cache size is changed'
    ...
    if ((Get-XMLConfigClientMaxLogSizeEnabled -like 'True') -eq $true) {
        ...
        Write-Log 'Validating Max CCMClient Log Size...'
    ...
    Write-Log 'Validating ClientSettings'
    ...
    if (($ClientWUAHandler -like 'True') -eq $true) {
		...
		Write-Log 'Validating Windows Update Scan not broken by bad group policy...'
    ...
    if ($UpdatesEnabled -like 'True') {
		...
		Write-Log 'Validating required updates are installed...'
		Test-Update -Log $log
	}

    Write-Verbose "Validating $env:SystemDrive free diskspace (Only warning, no remediation)..."
    Write-Log "Validating $env:SystemDrive free diskspace (Only warning, no remediation)..."
    Test-DiskSpace
    Write-Verbose 'Getting install date of last OS patch for SQL log'
    Write-Log 'Getting install date of last OS patch for SQL log'
    Get-LastInstalledPatches -Log $log
    Write-Verbose 'Sending unsent state messages if any'
    Write-Log 'Sending unsent state messages if any'
    ...
    Write-Log 'Getting Source Update Message policy and policy to trigger scan update source'
    ...
    if (($restartCCMExec -eq $true) -and ($Reinstall -eq $false)) {
        ...
        Write-Log "Restarting service CcmExec..."
        ...
        if ($SCCMLogJobs.Rows.Count -ge 1) {
            ...
            Write-Log "Processing changes to SCCM logfiles after remediation to prevent remediation again next time script runs."
            Update-SCCMLogFile
            Start-Service -Name CcmExec
        }
    ...
    Write-Log 'Cleaning up after healthcheck'
    CleanUp
    Write-Verbose 'Validating pending reboot...'
    Write-Log 'Validating pending reboot...'
    Test-PendingReboot -log $log
    Write-Verbose 'Getting last reboot time'
    Write-Log 'Getting last reboot time'
    ...
    if (Get-XMLConfigClientCacheDeleteOrphanedData -like "true") {
        ...
        Write-Log "Removing orphaned ccm client cache items."
	...
	Write-Log "ConfigMgr Client set to reinstall, but ccmsetup.exe is already running." 
	}
    
    elseif (($Reinstall -eq $true) -and ($null -eq $proc)) {
        Write-Verbose 'Reinstalling ConfigMgr Client'
        Write-Log 'Reinstalling ConfigMgr Client'
        Resolve-Client -Xml $Xml -ClientInstallProperties $ClientInstallProperties
...
End {
    ...
    Write-Log "Setting last ran to $($Date)"

    if ($LocalLogging -like 'true') {
        Write-Output 'Updating local logfile with results'
        Write-Log 'Updating local logfile with results'
    ...
    if (($FileLogging -like 'true') -and ($FileLogLevel -like 'full')) {
        ...
        Write-Log 'Updating fileshare logfile with results'
